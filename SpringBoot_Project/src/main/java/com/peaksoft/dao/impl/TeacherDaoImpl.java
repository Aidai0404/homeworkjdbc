...
public class TeacherDaoImpl implements TeacherDao {
    ...
    public List<Course> getCoursesByTeacherId(Long id) {
        List<Course>courses = entityManager.createQuery("select c from Course c join c.teachers t where t.id=?1").setParameter(1,id).getResultList();
        return courses;
    }
...
//SELECT customers FROM customers WHERE first_name LIKE 'Jo%';
